...
import { REVIEW_FIELD } from '@thp/shared';
...
/** One script. Keys are field names, so a script reads like the answer it stands in for. */
export type FakeDraftScript = Readonly<Record<string, string>>;
  ...
  return {
    ...
    async generate(request: GenerationRequest): Promise<GenerationResult> {
      ...
      for (const kind of request.kinds) {
        const value = script[REVIEW_FIELD[kind]];
        if (typeof value !== 'string' || value.trim() === '') {
          throw new GenerationError(
            `the fake script has no ${REVIEW_FIELD[kind]} in it, and one was asked for`,
          );
        }
