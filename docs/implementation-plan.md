...
- **The client holds no authorisation decision.** It hides what a Member cannot do; the API is what
  ...
  [slice-architecture.md § Next.js application — client half](slice-architecture.md#L106)).
- **Every `/api/v1` route requires a session**, with no exceptions
  ([3.1.2](prd.md#L44), [slice-architecture.md § Extension points](slice-architecture.md#L303)).
- **The original upload is never overwritten or deleted** — the one non-negotiable
  ([3.4.9](prd.md#L102), [slice-architecture.md § Media store](slice-architecture.md#L161)).
...
- **Correlation id spans API request → job → provider call**, and every gate transition is logged
  ...
  ([slice-architecture.md § Key choices](slice-architecture.md#L237)).
- **Do not build what is deferred.** Before adding infrastructure, check
  [slice-architecture.md § Deliberately deferred](slice-architecture.md#L321) — no broker, no CDN,
...
Router app, a managed Postgres with migrations applied by command, and a versioned `/api/v1` route
...
[slice-architecture.md § Next.js application — API half](slice-architecture.md#L120);
[slice-architecture.md § Primary datastore](slice-architecture.md#L174);
[slice-architecture.md § Key choices](slice-architecture.md#L237);
[architecture.md § Key technology choices](architecture.md#L213);
[5.2.2](prd.md#L706)
**Notes:** two decisions here are hard to walk back. The Postgres instance must be one where
pgvector is *available* even though it is not enabled ([slice-architecture.md § Primary
datastore](slice-architecture.md#L174)). And the client must call an absolute API origin over HTTP
with no server-module imports and no database access — that boundary is what makes the Capacitor
...
[slice-architecture.md § Data model (slice) → Accounts](slice-architecture.md#L186);
[slice-architecture.md § Extension points → Role enum + policy module](slice-architecture.md#L303);
[architecture.md § Cross-cutting concerns](architecture.md#L275)
...
[slice-architecture.md § Data model (slice) → Accounts](slice-architecture.md#L186);
[slice-architecture.md § Key choices](slice-architecture.md#L237) — "Two inputs this slice needs
...
([slice-architecture.md § Next.js application — API half](slice-architecture.md#L120)) — it must
...
[slice-architecture.md § Media store](slice-architecture.md#L161);
[slice-architecture.md § Data model (slice) → The spine](slice-architecture.md#L186);
[slice-architecture.md § Key choices](slice-architecture.md#L237) — "Two inputs", item 2;
...
later ([slice-architecture.md § Extension points](slice-architecture.md#L303)).
...
[slice-architecture.md § Worker process](slice-architecture.md#L136);
[slice-architecture.md § Job ledger (in Postgres, not a broker)](slice-architecture.md#L154);
[slice-architecture.md § Data model (slice) → Pipeline state](slice-architecture.md#L186);
[architecture.md § Worker pool](architecture.md#L151)
**Notes:** the largest step in the plan, and the one where the timestamped segment — the atom of the
whole system ([architecture.md § Data model](architecture.md#L175)) — gets its shape. The API
...
[slice-architecture.md § Data model (slice) → The review gate](slice-architecture.md#L186);
[slice-architecture.md § Worker process](slice-architecture.md#L136);
[architecture.md § Cross-cutting concerns](architecture.md#L275)
...
([slice-architecture.md § Extension points](slice-architecture.md#L303)).
...
([slice-architecture.md § Divergence from the north star](slice-architecture.md#L276)). Signed-URL
...
[slice-architecture.md § Data model (slice) → Member-owned state](slice-architecture.md#L186);
[slice-architecture.md § Extension points → Client-owned playback state](slice-architecture.md#L303);
[slice-prd.md § Slice flows → C](slice-prd.md#L210)
...
[slice-architecture.md § Extension points → `(recording_id, timestamp_ms)` offset](slice-architecture.md#L303)
...
Cross-checked against [slice-architecture.md § Deliberately deferred](slice-architecture.md#L321)
...
| [slice-architecture.md § Data model (slice)](slice-architecture.md#L186) | the `## Data model (slice)` heading | correct; sub-parts share the heading anchor and are named in the link text |
