# Teaching Hub — Epic implementation plan: core-listening

> Phase 5 artefact. The build order for [epic prd](docs/epics/epic-core-listening/prd.md#L1) under
...
> fits in your head. Phases 6–8 walk this list one ticket at a time.

## The arc

Five stories, in dependency order.

1. **Get a person in** (tickets 1–5). Nothing in this product is reachable without an authenticated
...
2. **Get a recording in and through the machine** (tickets 6–9). Upload straight to object storage,
...
3. **Put a human in front of the machine** (tickets 10–12). The review gate — the structure every
   later AI artefact passes through — and the publish decision that is the only route to member
   visibility.
4. **Give the member what they came for** (tickets 13–20). Library, player, resume-anywhere,
   follow-along transcript, correction, series.
5. **Put it where people can reach it** (ticket 21). The host provisioned, both processes supervised,
   TLS, migrations run against production, and backups with a proven restore. The only story that
   ships no behaviour — and the only one whose position in the order is arguable, for the reasons in
   the ticket itself.

Tickets 1–12 are admin-and-machine only; **no member can see anything until ticket 12**. That is
...
- **Responsive from one codebase.** Every screen works on phone, tablet and desktop — the
  responsive row of [§5.1](docs/project/prd.md#L689), per
  [epic prd § In scope → 8](docs/epics/epic-core-listening/prd.md#L161). No ticket ships a desktop-only screen. There is no
  separate "make it responsive" ticket, because there is no point at which it is acceptable not to be.
...
- **Every `/api/v1` route requires a session** — with no exception carrying content
  ...
  [epic architecture § Extension points](docs/epics/epic-core-listening/architecture.md#L323)). Health is outside the rule
  ...
  [Ticket 21](docs/epics/epic-core-listening/implementation-plan.md#L341) depends on one. The exception is an **allowlist in the route wrapper, not a
  convention**: Ticket 2 ships it as an enumerated list plus a test asserting every route *not* on that
  list refuses an anonymous request, so "no exceptions" can never quietly become "the exceptions
  nobody wrote down".

  > **Amended after ticket 2.** This constraint said *exactly one* exception, `GET /api/v1/health`,
  > held in the *session middleware*. Both were wrong on contact with the code, and the amended
  > [epic architecture § Extension points](docs/epics/epic-core-listening/architecture.md#L323) is the settled version.
  ...
  > not have that property. **Ticket 3 (invitation accept) and Ticket 4 (password reset) each add one
...
**References:** [epic architecture § Overview](docs/epics/epic-core-listening/architecture.md#L7);
...
not enabled ([epic architecture § Primary datastore](docs/epics/epic-core-listening/architecture.md#L177)), but
...
### Ticket 2 — Sessions and server-side authorisation
...
**References:** [epic prd § In scope → 1](docs/epics/epic-core-listening/prd.md#L35);
[3.1.1](docs/project/prd.md#L43); [3.1.2](docs/project/prd.md#L44); [3.1.5](docs/project/prd.md#L47);
[epic architecture § Data model (epic) → Accounts](docs/epics/epic-core-listening/architecture.md#L193);
[epic architecture § Extension points → Role enum + policy module](docs/epics/epic-core-listening/architecture.md#L323);
...
[epic prd § Rationale](docs/epics/epic-core-listening/prd.md#L241) names as making this epic throwaway if skipped.
...
**References:** [3.1.3](docs/project/prd.md#L45); [3.1.4](docs/project/prd.md#L46);
[epic prd § Epic flows → A](docs/epics/epic-core-listening/prd.md#L210);
[epic architecture § Data model (epic) → Accounts](docs/epics/epic-core-listening/architecture.md#L193);
...
**Notes:** [3.1.4](docs/project/prd.md#L46) says only "a fixed window". The settled value is **7 days**; tokens
are stored hashed.

### Ticket 4 — Account lifecycle: reset, deactivation, last-admin guard, profile
...
**References:** [3.1.6](docs/project/prd.md#L48); [3.1.7](docs/project/prd.md#L49); [3.1.11](docs/project/prd.md#L53);
[3.1.12](docs/project/prd.md#L54) (display name only — avatar deferred);
[epic prd § In scope → 1](docs/epics/epic-core-listening/prd.md#L35)
**Notes:** the last-admin invariant is enforced in the API, not the interface
([epic architecture § Next.js application — API half](docs/epics/epic-core-listening/architecture.md#L123)) — it must
...
tickets 3 and 4. One flat operator surface; the shell is where every later panel hangs.
**References:** [epic prd § In scope → 7](docs/epics/epic-core-listening/prd.md#L146);
[3.19.9](docs/project/prd.md#L437); [3.19.1](docs/project/prd.md#L429) (flat for now — nothing to gate until Contributor
exists); [epic prd § Epic flows → A](docs/epics/epic-core-listening/prd.md#L210)

---

## Story 2 — Get a recording in and through the machine

### Ticket 6 — Recording upload to object storage
...
[epic architecture § Data model (epic) → The spine](docs/epics/epic-core-listening/architecture.md#L193);
[epic architecture § Key choices](docs/epics/epic-core-listening/architecture.md#L255) — "Two inputs", item 2;
...
FLAC. `Recording` gets **no processed-media pointer**; adding one is what [§3.4](docs/project/prd.md#L88) does
later ([epic architecture § Extension points](docs/epics/epic-core-listening/architecture.md#L323)).

### Ticket 7 — Job ledger, worker process and transcription
...
[3.21.2.1](docs/project/prd.md#L483); [3.21.2.3](docs/project/prd.md#L485);
...
[epic architecture § Data model (epic) → Pipeline state](docs/epics/epic-core-listening/architecture.md#L193);
[project architecture § Worker pool](docs/project/architecture.md#L147)
**Notes:** the largest ticket in the plan, and the one where the timestamped segment — the atom of the
whole system ([project architecture § Data model](docs/project/architecture.md#L171)) — gets its shape. The API
enqueues through a **queue port** it does not look behind, so a broker can drop in later without
touching the ledger or the dashboard. Jobs are idempotent and individually re-runnable, which ticket 8
depends on.

### Ticket 8 — Pipeline status panel with per-step re-run
...
### Ticket 9 — Draft generation: summary and description
...
[3.6.1](docs/project/prd.md#L127); [3.6.2](docs/project/prd.md#L128); [4.17.1](docs/project/prd.md#L681) (description only — topics,
tags and scripture references deferred); [4.17.5](docs/project/prd.md#L685); [3.21.2.2](docs/project/prd.md#L484);
[epic architecture § Data model (epic) → The review gate](docs/epics/epic-core-listening/architecture.md#L193);
[epic architecture § Worker process](docs/epics/epic-core-listening/architecture.md#L139);
[project architecture § Cross-cutting concerns](docs/project/architecture.md#L271)
**Notes:** the `kind` column is the only thing in this epic built past its immediate need, and it is
...
## Story 3 — Put a human in front of the machine

### Ticket 10 — Pending Reviews: review, edit, approve, discard
...
**References:** [epic prd § In scope → 3](docs/epics/epic-core-listening/prd.md#L77);
[3.6.4](docs/project/prd.md#L130); [3.6.5](docs/project/prd.md#L131); [3.6.6](docs/project/prd.md#L132); [3.6.7](docs/project/prd.md#L133);
[3.6.10](docs/project/prd.md#L136); [4.17.2](docs/project/prd.md#L682); [4.17.5](docs/project/prd.md#L685);
[3.19.2](docs/project/prd.md#L430); [3.19.3](docs/project/prd.md#L431);
[epic architecture § Data model (epic) → The review gate](docs/epics/epic-core-listening/architecture.md#L193)
**Notes:** the in-app "ready for review" notification ([3.6.3](docs/project/prd.md#L129)) is deferred — the queue
is how an admin finds work in this epic.

### Ticket 11 — Regenerate with a steering prompt
...
[3.6.9](docs/project/prd.md#L135) is deferred with [§3.17](docs/project/prd.md#L361).

### Ticket 12 — Publish and unpublish
...
[3.2.2](docs/project/prd.md#L63); [3.2.11](docs/project/prd.md#L72); [3.6.11](docs/project/prd.md#L137); [3.6.12](docs/project/prd.md#L138);
[4.17.3](docs/project/prd.md#L683);
[epic architecture § Data model (epic) → The spine](docs/epics/epic-core-listening/architecture.md#L193)
**Notes:** publishing without a review gate is the second of the three things
[epic prd § Rationale](docs/epics/epic-core-listening/prd.md#L241) names as making this epic throwaway. Emit a domain
event on publish; nothing subscribes yet.

---

## Story 4 — Give the member what they came for

### Ticket 13 — Member library and recording page
...
### Ticket 14 — Streaming playback and scrubbing
...
[epic architecture § Media store](docs/epics/epic-core-listening/architecture.md#L164);
[epic prd § In scope → 8](docs/epics/epic-core-listening/prd.md#L161); [§6](docs/project/prd.md#L724) Security
...
### Ticket 15 — Playback speed that persists
...
### Ticket 16 — Resume position across devices
...
[epic architecture § Data model (epic) → Member-owned state](docs/epics/epic-core-listening/architecture.md#L193);
[epic architecture § Extension points → Client-owned playback state](docs/epics/epic-core-listening/architecture.md#L323);
[epic prd § Epic flows → C](docs/epics/epic-core-listening/prd.md#L210)
**Notes:** client-owned-and-pushed is the shape that makes offline ([§3.18](docs/project/prd.md#L391)) an addition
rather than a rewrite — it is [epic prd § Rationale](docs/epics/epic-core-listening/prd.md#L241)'s stated check that this
cut is not a dead end. Listening history and the completed marker are **out**; resume position is
the only playback state this epic keeps.

### Ticket 17 — Follow-along transcript
...
[epic architecture § Extension points → `(recording_id, timestamp_ms)` offset](docs/epics/epic-core-listening/architecture.md#L323)
...
### Ticket 18 — Transcript correction and the regeneration offer
**Delivers:** an admin corrects segment text on a published recording — recording who corrected it
and when — and is offered regeneration of the summary, which routes through ticket 11's path. Member
...
### Ticket 19 — Series: create, rename, assign, move
...
**References:** [epic prd § In scope → 4](docs/epics/epic-core-listening/prd.md#L100);
[3.3.2](docs/project/prd.md#L79); [3.3.6](docs/project/prd.md#L83) (create/rename/move only — reorder and merge deferred);
[3.19.5](docs/project/prd.md#L433) (minus artwork upload);
[epic architecture § Data model (epic) → The spine](docs/epics/epic-core-listening/architecture.md#L193)

### Ticket 20 — The series view
...
[epic prd § Epic flows → C](docs/epics/epic-core-listening/prd.md#L210)
**Notes:** depends on ticket 16 — the per-recording progress shown here is the same
`playback_progress` row the player writes, which is why series lands after the player rather than
next to the library.

## Story 5 — Put it where people can reach it

### Ticket 21 — Production deployment
**Delivers:** the epic runs on the host it will live on, and survives a reboot. The VPS provisioned;
...
the box rather than in the repository; ticket 1's migration command applied against production by the
...
[epic architecture § Overview](docs/epics/epic-core-listening/architecture.md#L7);
[epic architecture § Primary datastore](docs/epics/epic-core-listening/architecture.md#L177);
[01-project-skeleton.md § Assumptions to confirm](docs/epics/epic-core-listening/stories/get-a-person-in/01-project-skeleton.md#L99) — items 1 and 9,
both settled to this host; [§6](docs/project/prd.md#L724) — *Storage* ("nothing expires") is what makes the
restore drill part of the ticket rather than an afterthought, and *Security* ("media storage is not
publicly addressable") is what the object-store configuration has to hold.
**Notes:** three things worth naming. **This ticket's position is the one genuinely debatable ordering
in the plan.** It depends on nothing after ticket 1, and pulling it forward to ticket 2 would mean every
later ticket is validated against real infrastructure — with deployment problems arriving one at a time
instead of twenty at once, and ticket 6's presigned uploads in particular far easier to debug against a
real origin and a real bucket. It sits last because there is no member to serve until ticket 20 and a
deployment carried through twenty tickets of schema churn is real cost. That tradeoff deserves a
decision, not inheritance. Second, this ticket adds **no CDN and no broker**: signed URLs still point
straight at the object store, and the box runs exactly the two processes the epic already has
([epic architecture § Deliberately deferred](docs/epics/epic-core-listening/architecture.md#L341)). Third,
`NEXT_PUBLIC_API_ORIGIN` moves from `http://localhost:3000` to the real origin here — the single
value [01-project-skeleton.md](docs/epics/epic-core-listening/stories/get-a-person-in/01-project-skeleton.md#L112) assumption 2 exists to keep cheap.
...
and [epic prd § Still remaining after this epic](docs/epics/epic-core-listening/prd.md#L171). No ticket below exists, and
...
| :---- | :---- | :---- |
| [3.5.2](docs/project/prd.md#L113) (ticket 7) | "The transcript is segmented and timestamped…" | correct |
| [4.17.3](docs/project/prd.md#L683) (ticket 12) | "Nothing publishes automatically…" | correct |
| [3.19.4](docs/project/prd.md#L432) (ticket 8) | "A processing status view showing which recordings are in processing, transcription or generation, and which have failed" | correct — the processing column is the part this epic omits |
| [3.1.11](docs/project/prd.md#L53) (ticket 4) | "At least one Admin account must exist at all times…" | correct |
| [3.2.4](docs/project/prd.md#L65) (ticket 15) | "Playback speed is adjustable across 0.5x, 0.75x, 1x, 1.25x, 1.5x and 2x, and the chosen speed persists across recordings" | correct — confirms all six steps |
...
| [epic prd § In scope → 5](docs/epics/epic-core-listening/prd.md#L117) (tickets 14–16) | `### 5. The player` | correct |

One thing worth flagging rather than leaving to be discovered mid-build: [3.5.1](docs/project/prd.md#L112) reads
"once audio processing (3.4) completes", and ticket 7 triggers on **upload** completing instead. That
is [epic prd § In scope → 2](docs/epics/epic-core-listening/prd.md#L52)'s single deliberate narrowing, not a
mis-reference — but a ticket-planning session that read [3.5.1](docs/project/prd.md#L112) alone would get it wrong,
which is why ticket 7 cites both.
