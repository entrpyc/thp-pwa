# Teaching Hub — Epic architecture: core-listening

> Phase 4 artefact. The architecture for **this epic only** — deliberately smaller than
> [project architecture](docs/project/architecture.md#L1). Where it knowingly differs from the north star, that is
...
[project architecture § Estimated running costs](docs/project/architecture.md#L343), all three run on a single host.
...
Nothing — this is the first epic. `docs/completed-epics/` is empty and the repository holds no
application code, so there are no extension points to land on and no existing structure to change.
Every component below is new, which is why **Changes to existing structure** is empty and the epic
diagram has no *untouched* nodes.

The consequence worth stating: this epic is the one that *creates* the extension points every
later epic will read. That is what the **Extension points** section is for, and it is the section
the next epic's first tickets will open first.
...
**Legend.** Green = **adds**, new in this epic. Orange = **changes**, existing structure reshaped
here — *empty this epic*. Grey = **untouched**, already running — *empty this epic, nothing runs
...
The responsive React UI from one codebase ([epic prd § In scope → 8](docs/epics/epic-core-listening/prd.md#L161)),
...
- **Owns:** sessions and password hashing, invitation issue/accept/revoke/resend, role enforcement
  on every request ([3.1.5](docs/project/prd.md#L47)), the last-admin invariant ([3.1.11](docs/project/prd.md#L53)), upload
...
jobs against the ledger ([project architecture § Worker pool](docs/project/architecture.md#L147)).
...
[project architecture § Key technology choices](docs/project/architecture.md#L209) marks the single-datastore decision
...
object store, proven by a restore — [implementation plan § Ticket 21](docs/epics/epic-core-listening/implementation-plan.md#L341)
owns that, along with the rest of the deployment.

## Changes to existing structure

**None — nothing is running yet.** This section exists in the template because from the next epic on it
is where the regressions live; for this epic an empty answer is the correct one.
...
> makes sign-out real rather than a request that the browser forget something, and what lets ticket
> 4's deactivation ([3.1.7](docs/project/prd.md#L49)) end a session that is already open instead of waiting for
...
| Choice | Why, for this epic | Relation to the north star |
| :---- | :---- | :---- |
...
| **Correlation id spanning API request → job → provider call, plus error tracking** | Cheap now, and a pipeline that fails silently in this epic is a pipeline nobody trusts three epics later. The dashboard reads the ledger, not the logs. | Matches [project architecture § Cross-cutting concerns](docs/project/architecture.md#L271). |

**Two inputs this epic needs and nothing defines** — carried from [epic prd § Duplicate &
reference audit](docs/epics/epic-core-listening/prd.md#L289). Neither blocks starting; both are settled here so the build is
...
Two, both *the epic bending*, neither touching anything [project architecture](docs/project/architecture.md#L1) marks
**expensive to reverse**.

> **One divergence was resolved rather than reversed.** This epic previously diverged by running
...
The seams this epic leaves. **This is the section the next epic reads first.**
...
- **Read replicas, multi-region, sharding, CQRS, event sourcing, real-time transport.** Named in
  [project architecture § Scalability](docs/project/architecture.md#L317) as not-yet, and nothing here changes that.
