...
> Phase 6 artefact for [implementation plan § Ticket 2](docs/epics/epic-core-listening/implementation-plan.md#L95).
> Sections pulled: [epic prd § In scope → 1](docs/epics/epic-core-listening/prd.md#L35);
> [3.1.1](docs/project/prd.md#L43), [3.1.2](docs/project/prd.md#L44), [3.1.5](docs/project/prd.md#L47);
> [epic architecture § Data model (epic)](docs/epics/epic-core-listening/architecture.md#L193);
...
> [style-guide.md](docs/design%20referencess%20png/style-guide.md), standing in for the absent auth
> design reference by operator decision.

Two things in this ticket are structural rather than featural, and both are named as expensive to
skip. The **single `(actor, action, resource)` evaluation point** is one of the three structures
[epic prd § Rationale](docs/epics/epic-core-listening/prd.md#L241) says makes this epic throwaway if missed. The
**enumerated unauthenticated allowlist** is what turns [3.1.2](docs/project/prd.md#L44) from a convention into
...
Ticket 2 is the first ticket with a user-facing screen, and there is no auth PNG — no
...
[the style guide](docs/design%20referencess%20png/style-guide.md) instead**, which is a deliberate
carve-out from [CLAUDE.md § Designing pages](CLAUDE.md) for this ticket, not a precedent for
...
reference" is checkable against: tokens, not pixels. Ticket 2 therefore also lands the **token layer**
...
**The refusal seam** — the checkable half of [3.1.2](docs/project/prd.md#L44)
...
- The sweep test fails when deliberately violated — verified by a fixture route declared without
  ...
  guard, the same rule [Ticket 1](docs/epics/epic-core-listening/stories/get-a-person-in/01-project-skeleton.md) applied to the import boundary.
...
- All authorisation decisions resolve through one module — verified by a source-level check that no
  ...
  Ticket 1's import-boundary guard.
...
- A refusal for an authenticated caller is distinguishable from a refusal for an anonymous one —
  ...
  [project architecture § Cross-cutting concerns](docs/project/architecture.md#L271) makes error types part of the
  contract.
- Adding a role to the enum does not compile until every policy case handles it — verified by an
  ...
  widened cases" ([epic architecture § Extension points](docs/epics/epic-core-listening/architecture.md#L323)) rather
...
- No component declares a raw hex colour, a pixel radius or an ad-hoc spacing value — verified by a
  ...
  same style as Ticket 1's import-boundary guard. This is the check that makes "built from the style
...
1. **The allowlist has two entries in this ticket, not one.**
   [epic architecture § Extension points](docs/epics/epic-core-listening/architecture.md#L323) names
   ...
   "no unauthenticated route *carrying content*" — the property it was actually protecting. Ticket 3
   (invitation accept) and Ticket 4 (password reset) each add an entry, and each addition is a
...
2. **Sessions are server-side records, not signed stateless tokens.** Assumed: a `session` table
   ...
   what lets Ticket 4's deactivation ([3.1.7](docs/project/prd.md#L49)) end a live session instead of waiting
   ...
   [epic architecture § Data model (epic)](docs/epics/epic-core-listening/architecture.md#L193), which lists no such
   table — confirm it or overrule it now.
3. **Session lifetime.** Nothing in the PRD or epic architecture fixes one. Assumed: 30 days
...
4. **Password hashing.** Assumed: **argon2id** via `@node-rs/argon2` (prebuilt binaries, no node-gyp
   ...
   [netcup VPS](docs/project/architecture.md#L343) — a box that also runs Postgres and the worker, so the
   parameters are a real budget rather than a copied default. bcrypt is the fallback if the native
   package proves awkward.
5. **`user` columns arrive with the tickets that use them.** Assumed: this ticket ships `email`,
   `password_hash`, `display_name`, `role`, `created_at`, `updated_at` only. `deactivated_at` comes
   with Ticket 4, `preferred_playback_speed` with Ticket 15 — the same rule Ticket 1 set for tables.
6. **The first admin is seeded by a command, not by a migration.** Assumed: an idempotent CLI
   ...
   [Ticket 21](docs/epics/epic-core-listening/implementation-plan.md#L341).
...
8. **The `/api/v1/diagnostics/*` routes become authenticated.**
   [epic architecture § Extension points](docs/epics/epic-core-listening/architecture.md#L323) is explicit that they
   are not an exception. Assumed: they require a session like everything else, and Ticket 1's
   integration tests gain a sign-in helper. Consequence worth stating plainly: **Ticket 1's suite
   changes in this ticket**, which is expected, not a regression.
...
10. **No rate limiting or account lockout in this ticket.** Neither appears in [§3.1](docs/project/prd.md#L31)
    and neither has a named home in a later ticket. Assumed out of scope, with the cheap parts kept
...
Ticket 1's now-authenticated diagnostics tests need; the CSS token layer built from
[the style guide](docs/design%20referencess%20png/style-guide.md); and the sign-in screen and a
sign-out control composed from those tokens.

**Out:** invitations, accept, revoke, resend (Ticket 3). Password reset, deactivation, the last-admin
guard ([3.1.11](docs/project/prd.md#L53)) and profile editing (Ticket 4) — note the last-admin invariant belongs
at the data layer per [project architecture § Cross-cutting concerns](docs/project/architecture.md#L271), so it is
not a policy case this ticket should anticipate. Admin console and user management (Ticket 5). Any email
...
(30 days rolling, refreshed at most hourly on use), 5 (`user` columns for this ticket only), 6
...
- [epic architecture § Data model (epic)](docs/epics/epic-core-listening/architecture.md#L193) gains `session`.
- [epic architecture § Extension points](docs/epics/epic-core-listening/architecture.md#L323) — the unauthenticated
