...
- **3.1.2** The browser re-encodes the chosen image before sending it: **aspect ratio is preserved**, the longest edge is reduced to at most 2000 px if it exceeds that, and the result is one WebP under 2 MB. What is stored is that re-encoded image and not the file the admin picked — there is one object per series and no original beside it. (refines project prd 3.3.3)
...
- **The bound at 3.1.2 is a longest edge of 2000 px, WebP, under 2 MB.** — minor. Generous enough that a square upload survives at a resolution project prd 5.3.2 would accept, small enough to serve a thumbnail from.
