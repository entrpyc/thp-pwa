...
- ✅ **3.1.17** Registration is the one place the product tells an anonymous caller that an address already has an account. Everywhere else that would be an enumeration oracle and is refused as one — sign-in answers identically for a wrong password and an unknown address (3.1.1), and a reset request answers identically whether or not the address is a member (3.1.6). Here it is unavoidable: an address that already has an account cannot be given a second one, and the alternative is a screen that reports a success that did not happen. The disclosure is bounded to that single fact, and is bounded again by 3.1.18 — it is why this route, alone in the product, has a rate limit.
- ✅ **3.1.18** Registration is rate-limited. It is the only unauthenticated route that creates anything, and the only one that answers an anonymous caller with a fact about an account (3.1.17), so it is the first place where repetition is itself the abuse; the others are 3.1.20 onward, and the full account of every budget the product holds is [rate-limits.md](rate-limits.md). Two budgets apply over the same window: one per caller, which stops a single machine from working through a list of addresses or filling the member list; and one across the whole route, which is the backstop for the same thing attempted from many addresses at once, because a per-caller budget cannot see that at all. A refused request creates nothing, costs nothing further to the caller who is already over budget, and answers with the wait before another will be accepted rather than leaving a person to guess. Both budgets are deployment configuration with defaults, because how many members sit behind one address is a fact about a congregation rather than about the software.
- ✅ **3.1.19** The per-caller budget is deliberately set well above any honest burst — a room of members registering together on one wifi is a single caller as far as a rate limit can tell, and refusing the twentieth person in that room would be the product breaking rather than defending itself. The consequence is accepted and named here rather than discovered: while the whole-route ceiling is spent, registration is closed to everyone, so a sustained distributed flood can deny sign-up for as long as it is paid for. That is the deliberate trade — a few hours of "try again later" is recoverable, and ten thousand junk accounts in the member list is not — and reaching the ceiling is recorded as an operational event for an admin to act on, not as one refusal among many.
- ✅ **3.1.20** Sign-in is rate-limited per caller and per account, so that an online guessing run is bounded whether it comes from one machine or many, and so that the deliberate cost of verifying a password (3.1.13) is spent only on attempts the budget allowed. The account budget is spent whether or not the address has an account, so a refusal says nothing 3.1.1 keeps quiet; and it is keyed by a hash of the address, so neither the limiter nor the log holds an address a probe supplied. A caller refused by their own budget does not spend the account's, so a machine that is already blocked cannot lock a real member out. A refused attempt verifies nothing and answers with the wait. There is no lockout and nothing for an admin to clear: the window expires by itself. Both budgets are deployment configuration with defaults, set generously enough that a member who knows their password never meets them.
- ✅ **3.1.21** A password-reset request is rate-limited per caller, with a ceiling across the route. The one-message-a-minute rule in 3.1.6 bounds what a single account can be sent; the budget bounds what a single caller, or many callers together, can make the product send at all. A refusal is an answer about the caller's request count and never about the address in the request, so the identical-response rule in 3.1.6 is kept. While the route's ceiling is spent, reset requests are closed to everyone, for the same trade 3.1.19 accepts on registration; reaching it is recorded as an operational event. Both budgets are deployment configuration with defaults.
- ✅ **3.1.22** Feedback and every upload grant are budgeted per account, at rates no honest use reaches, so that a signed-in member cannot make the product send mail or mint storage on a loop. The budget is spent on what would have left the server — a message, a URL — so a request refused on its merits costs nothing against it; each kind of grant is its own budget, so an admin's recordings and their avatar cannot lock each other out. These numbers are constants rather than configuration, because how often a person reports a bug or retries a picture does not depend on where the product is deployed.
...
- ✅ **3.2.16** An admin can correct a recording's title and the date it was recorded after it has been uploaded, from the row it is managed on in the console (3.19.3). Both are set by hand at upload (4.2) — typed in a hurry, from a name heard once — so both are correctable afterwards, and the correction changes those two fields and nothing else: the audio, the transcript, the summary and its own publication state, the chapters, the scripture references, the series and every member's saved position (3.2.5) all survive it untouched, and nothing is re-processed. It is available whether or not the recording is published: a misheard name is discovered after members start listening as often as before, and taking a teaching down to fix a spelling would cost an outage for a typo. Publication is unaffected in both directions — correcting a live teaching leaves it live and members see the new title immediately, and correcting a draft does not publish it. Because the date recorded is the sort key (3.3.1, 3.3.4, 4.2), correcting it moves the recording in the library, in the console list and within its series. Replacing the **audio** is a different act with different consequences and remains 3.2.10.
...
- ✅ **3.3.4** A series view lists every recording in that series newest recorded first — the same order as the date-ordered library at 3.3.1, so the product has one answer to "what is most recent" wherever a member meets a list of teachings — numbered by position in the study, so the oldest is `01` and the numbering counts down the list, with the member's own progress (3.2.5) shown per recording.
...
*The step and its plumbing came first — every upload transcoded into a playback rendition for exact seeking (3.4.12), the original kept, playback preferring the rendition, a failure surfacing on the pipeline view. The sound work followed as a filter chain inside that same step (3.4.2–3.4.4), governed by one versioned profile (3.4.5) that an admin tunes from a console panel, hears before saving (3.4.6), and applies to what is already processed only by choosing to (3.4.7). What remains is re-processing a chosen batch from the console (3.4.8) and the podcast side of 3.4.10.*

**Functional requirements**

- ✅ **3.4.1** Every uploaded recording is automatically processed before it becomes available for playback. Processing is the first step of the pipeline (3.21.1), ahead of transcription, so the rendition members will hear exists before anything downstream spends money; transcription runs on the step completing (3.5.1). What the step does today is the transcode of 3.4.12 and nothing else.
- ✅ **3.4.2** Processing reduces background noise present in the raw recording. It is spectral noise reduction that tracks the noise floor as the recording goes rather than assuming one, so the profile says how much to take out and each recording says from where — which is what a library recorded in several rooms needs. The amount is the profile's first knob, in decibels; at zero the stage is skipped altogether.
- ✅ **3.4.3** Processing enhances voice clarity and intelligibility. The stage is three things done together: a high-pass that removes handling rumble and room boom below the voice, a lift of the presence band around three kilohertz by the profile's second knob in decibels, and a gentle two-to-one compressor that brings quiet phrases up towards the loud ones without makeup gain, because level is the next stage's job. At zero all three are skipped, so a recording that is already clear is left alone.
- ✅ **3.4.4** Processing normalises loudness so volume is consistent from one teaching to the next, including across recordings captured months apart in different rooms. It is EBU R128 normalisation to the profile's integrated target, the third knob, in LUFS, at a fixed true-peak ceiling of −1 dBTP so a lossy encode cannot clip, and a fixed loudness range of 11 LU, the broadcast figure for speech. It runs in two passes: the first measures the recording as it comes out of the two stages above, the second normalises linearly against that measurement, which keeps the dynamics of speech intact where a single pass would pump on every pause. It is the last stage and is never skipped: what every recording shares is its level.
- ✅ **3.4.5** A single named sound profile defines the processing applied to all recordings, so the library has one consistent character rather than per-recording tuning. The profile is three knobs over a fixed chain, not a filter graph an admin edits: noise reduction from 0 to 30 dB, voice clarity from 0 to 6 dB, and a loudness target from −24 to −12 LUFS, each a whole number the form, the API and the database refuse outside its range for the same reason. A chain a person can type into is a chain a typo can break for every upload until somebody notices; a knob can only be turned too far, and too far is refused. Version 1 is seeded with the product's defaults (12 dB, 3 dB, −16 LUFS) so there is never a moment when a recording could be processed under no profile, and the version in force is read once at the start of an encode and written beside the rendition it made (4.2), so a save during a long encode changes nothing about that encode.
- ✅ **3.4.6** Admins can configure the sound profile's settings and preview the effect on a sample recording before saving changes. Both live on a console panel of their own (3.19.17). The preview is thirty seconds of any recording in the library — from a minute in unless the admin names a start, clamped to the transcript's end when the teaching is shorter — rendered twice by the worker: once plain, and once under whatever the knobs say at that moment, saved or not. Both are fresh encodes of the same thirty seconds, so what differs between the two players is the profile and nothing else. A preview is a job in the ledger like any other, so it is claimed, reclaimed after a crash and readable by an operator, and it is a step the pipeline does not contain, so nothing follows it; the console asks after it every two seconds until it is heard or has failed with a reason. One preview per recording at a time: a second request while one renders is refused rather than answered with the first one's settings.
- ✅ **3.4.7** Changing the sound profile does not retroactively alter already-processed recordings unless an admin explicitly re-processes them. Saving the profile writes a new version and never edits one, because every recording records the version that produced its rendition and that number has to keep meaning what it meant; a save that changes no knob is refused rather than numbered. The profile panel lists every recording with the version it was processed under — or that it was processed before the profile existed, or that it has no rendition yet — and offers to re-process one, which produces its rendition again under the version in force and changes nothing else: the transcript, its corrections, the drafts and the chapters are untouched, because the transcript's timings describe the original and the original is unchanged. The re-process is refused while a rendition of that recording is already being written by either step, so two encodes cannot race to point the row.
- 🔨 **3.4.8** Admins can re-run processing on any individual recording or on a batch of recordings. Two individual re-runs exist, and they are different acts on purpose. From the pipeline view (3.19.4), "Process audio · Run again" carries the cascade 3.21.2.7 describes — the recording is transcribed again, its corrections are discarded, its drafts and chapters regenerated — which is why the press asks for confirmation naming all of that. From the profile panel (3.19.17), "Re-process" produces only the rendition (3.4.7). The two are different routes rather than one route with a flag, because a difference like that in a request body is a difference that gets lost. Re-running a chosen batch from the console is not yet built; the command of 3.4.13 remains the way to bring many recordings forward at once.
- ✅ **3.4.9** The original uploaded file is retained unmodified alongside the processed output, so processing can be re-run or reverted. The step never writes to the original: it is what the transcript's timings describe and what transcription still reads. A re-run writes a new rendition under a new key and repoints the recording, and the superseded rendition stays in storage, unreferenced — the same accepted price replaced artwork pays (3.3.3).
- 📝 **3.4.10** The processed output serves both in-app streaming and podcast distribution (5.3) without a separate export step.
- ✅ **3.4.11** If processing fails, the recording is flagged in the admin dashboard (3.19.4) rather than silently published or silently dropped. The failed job carries a sentence an operator can act on — that the original could not be fetched, that the encoder is not installed on the worker host, that it exited with an error and what it last said, that it did not finish inside the thirty minutes it is given, or that it reported success and stored nothing — and the pipeline halts there (3.21.2.3), so nothing is transcribed or drafted from a recording without a rendition.
- ✅ **3.4.12** The rendition is AAC audio in an MP4 container, encoded at a constant 96 kbit/s with the seek index moved ahead of the media. The container is the point rather than the codec: browsers seek an MP4 exactly, because it carries a sample-to-time index, where a variable-bitrate MP3 is seeked by estimation — measured on a real teaching, the position reported after a seek was up to nine seconds away from the audio actually playing, and every anchor on the clock (captions 3.5.10, note markers 3.12.12, chapter boundaries 3.22.17) inherited the error while the transcript's own timings were right to within a frame. Moving the index to the front lets a browser seek before it has the whole file, which is what streaming a ninety-minute teaching is. 96 kbit/s is transparent for speech and roughly a quarter smaller than a typical original; it is constant so an encode's size is predictable, and it is one named number to revisit the day the library carries music. Cover art embedded in an upload is dropped rather than carried into an audio-only container, because carrying it is what failed on exactly the files a person exports from a phone app.
- ✅ **3.4.13** Playback streams the rendition when one exists and the original otherwise (3.2.13), so a recording uploaded before the step existed keeps playing. Those recordings are brought forward by a backfill command an operator runs on the worker host. It walks every recording without a rendition, oldest first, and produces each one's rendition **outside the pipeline** — deliberately, because a step that succeeds inside it enqueues transcription, and a backfill that did so would spend the provider on every teaching and replace every transcript and its corrections in the library. It resumes where it stopped, since only recordings still without a rendition are listed, and it stops at the first failure rather than repeating one mistake at scale. It processes under the profile in force (3.4.5) and records the version, exactly as the step does; one recording at a time, the same thing is the console's "Re-process" (3.4.7).
- ✅ **3.4.14** The encoder is local tooling on the worker host rather than a paid service: which one, and where its binary lives, is deployment configuration with a default, and the step spends nothing against the daily ceiling (3.21.2.8). It is still evidenced like a paid step: the job (4.18) records which tool produced which object at what size against the original's, which is what an operator reads when a rendition sounds wrong. The bytes never pass through the queue or the application — the worker brokers short-lived signed grants and the encoder moves the file between storage and itself — and the recording is pointed at a rendition only after storage confirms the object is there. The test suite runs the same step against a stand-in that copies the original unchanged, so the pipeline's shape is exercised on machines with no encoder installed.
...
- ✅ **3.19.3** Recording upload and the metadata review form (4.17.2), including the publish action (3.2.2). Each recording's row also carries the corrections an admin makes by hand rather than through the review queue: its title and the date it was recorded (3.2.16), and the series it belongs to (3.3.2). Neither is an entry in Pending Reviews, because neither is something a machine proposed for a person to approve.
...
- ✅ **3.19.16** The processing view shows what paid work has cost today against the day's ceiling (3.21.2.8), split by the step that spent it, and an admin can raise the ceiling for the rest of that day, with the raise recorded against them and shown to the next admin who looks. The raise is the whole ceiling for the day rather than an increment, cannot go below the configured default, and is capped so a mistyped amount cannot become an open budget. A re-run or a regenerate that the ceiling would refuse is refused at the press, with the same numbers, rather than a minute later as a failed row.

- ✅ **3.19.17** A sound profile panel (3.4.5–3.4.7): the version in force with who saved it and why, the three knobs as sliders with Save disabled while nothing has changed, a preview of any recording under the knobs as they stand (3.4.6) heard as two players side by side, and every recording with the version that processed it and a control to re-process that one recording's rendition alone. It refreshes itself while a preview or a re-process is running and stops asking once nothing is. Admin only, at every route behind it as well as at the panel.
...
- ✅ **3.21.2.8** Paid steps run only while the day's spend is under a ceiling. What a step cost is already recorded on its job (3.19.13), so the day's spend is the sum of those records since the UTC day began, and a step reached after the ceiling fails naming the ceiling, spends nothing, and is re-run through 3.21.2.4 once the ceiling is raised (3.19.16) or the day ends. The check is made on the total before a step starts, so a step that begins under the ceiling is allowed to finish over it; the true cap is the ceiling plus one step, stated rather than hidden. The ceiling is a deployment setting with a default, and the raise is bounded so it cannot be mistyped into no ceiling at all.
...
| Processed audio      | Auto-generated               | The playback rendition of 3.4.12, streamed in preference to the original (3.4.13); a recording without one streams the original |
| Sound profile version | Auto-set                    | Which version of the profile (4.20) produced the rendition, written in the same statement as the rendition's key (3.4.7). Null for a recording with no rendition, and for one made before the profile existed — never defaulted to 1, because that would be a claim about bytes nobody made                                                                                               |
...
*One row per version of the profile every recording is processed under (3.4.5), and the record a recording's version pointer (4.2) refers to.*

| Field                | Set by     | Notes                                                                                                                                                          |
| :------------------- | :--------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Version              | Auto-set   | 1 for the seeded default, one higher for each save. Unique, computed inside the insert so two admins saving at once cannot both take the same number             |
| Noise reduction (dB) | Admin-set  | 0–30, whole numbers (3.4.2). Held by the database as well as the form                                                                                          |
| Voice clarity (dB)   | Admin-set  | 0–6, whole numbers (3.4.3)                                                                                                                                     |
| Loudness target (LUFS) | Admin-set | −24 to −12, whole numbers (3.4.4). True peak and loudness range are constants of the product, not fields                                                     |
| Note                 | Admin-set  | Optional, at most 200 characters — why this version, shown beside it on the panel (3.19.17)                                                                    |
| Saved by / saved at  | Auto-set   | Null author for the seeded version and for an account since removed                                                                                            |

A version is never edited or deleted: saving is appending, so the number on every recording keeps meaning what it meant (3.4.7). The version in force is the highest one, a fact of the table rather than a flag two writers could disagree about.
...
- **7.1.6** Transactional email delivery, for the two flows that cannot complete without it: invitation (3.1.3) and password reset (3.1.6). **Resend**, sent over SMTP so the provider is configuration rather than code. It is named here because the choice is settled and consequence-free: no notification in 3.17 uses email — both channels there are push (3.17.1) and in-app (3.17.2) — so email volume never grows with content or activity, only with new accounts. Dozens of messages a month at 100 members and at 1,000, which sits inside Resend's free tier at both ends.
- **7.1.7** Web platform capabilities the PWA leans on directly: service workers and local storage for offline (3.18), background audio with lock-screen controls (3.2.6), and web push (3.17.1).
- **7.1.8** External dependencies outside our control: Spotify's podcast ingestion, the Instagram, TikTok and LinkedIn publishing APIs (3.20), a source of Bible verse text (3.7.4), and the Apple and Google app store review processes (5.2).

### 7.2 Hard parts & unknowns

- **7.2.1** **App store acceptance.** Apple applies a minimum-functionality standard to web-wrapped applications. Rejection would not affect the product itself but would remove a distribution channel the product currently assumes, so 5.2 carries real risk.
- **7.2.2** **Push notifications across delivery routes.** Web push on iOS behaves differently from push in a store-packaged build, and the two may not be reachable through one mechanism. If they are not, 3.17.1 costs more than it appears to.
- **7.2.3** **Generative video quality, cost and turnaround.** This is the least proven capability in the product. If output quality or per-video cost disappoints, 3.11 changes shape — the feature survives but the workflow around it may need to lean far harder on manual selection.
- **7.2.4** **Offline storage limits on mobile.** Browser-managed storage can be evicted under pressure and is capped on iOS. "Download all" for a long series (3.18.6) may collide with that ceiling. **Answered for the browser route**: the download is refused before it starts rather than failing partway, against what the device reports free (3.18.17). Store builds hold downloads where the OS will not evict them (project tdd 4.3), so the risk is the browser's alone.
- **7.2.5** **Bible text licensing.** Displaying full verse text (3.7.4) is a licensing question, not a technical one, and it differs sharply by translation. **Answered for the translation the product runs on**: a free-use source and one configured translation (3.7.9), with verses rendered inline. It re-opens only if the ministry wants a translation that is not free to use — most of the well-known ones are not — which would be a licensing negotiation before it is a code change.
- **7.2.6** **Cross-referencing cold start.** Similarity between teachings (3.9) is only as useful as the library is large. Early on it will surface little, and the feature will look weaker than it is until the back catalogue is processed (3.21.3).
- **7.2.7** **Transcription accuracy on ministry-specific language.** Names, places and theological terminology are where speech-to-text degrades, and every downstream artefact inherits that error. The admin correction path (3.5.5) is a genuine requirement, not a convenience.
- **7.2.8** **Cost at back-catalogue scale.** Running years of recordings through transcription, summarisation and embedding is a one-off cost concentrated in a short window (3.21.3.3), and needs to be sized before committing.

### 7.3 Since settled, and recorded here because the product now runs on them

- **7.3.1** Speech-to-text is Deepgram Nova-3 and drafting is MiniMax M3, both named above; transactional email is Resend over SMTP. Media is held in S3-compatible object storage that is never publicly addressable, written by the browser under a presigned upload grant and read only through a short-lived signed URL. The application, its API and the job orchestration are one codebase plus a single worker process, and the job ledger is itself the queue — there is no broker, which is what makes enqueueing a step transactional with the write that caused it and makes the status view at 3.19.4 one query rather than a log read. Persistence is a single PostgreSQL database with pgvector installed but not yet enabled, so cross-referencing and search (3.9, 3.10) can be built on the datastore that already holds the content and its access rules. Each of these sits behind a narrow adapter, which is what keeps replacing one of them a one-file change rather than a re-architecture.

### 7.4 Settled since, and recorded in the TDD rather than here

- **7.4.1** Four things this section once left open now have decisions: the PWA is packaged for both stores with Capacitor (project tdd 4.3), search needs no infrastructure beyond the datastore above (project tdd 4.4, 4.5), offline synchronisation resolves conflicts per entity (project tdd 5.3), and embeddings are 1536-dimensional (project tdd 4.4).
