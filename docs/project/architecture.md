...
[3.9](docs/project/prd.md#L179) is correctly described in the PRD as infrastructure rather than a feature.
...
"a failure in AI generation never blocks listening" ([§6](docs/project/prd.md#L724)) structural rather than
aspirational — and it is also why the one unauthenticated surface in the product (`Feed`, serving
the podcast RSS at [5.3.1](docs/project/prd.md#L718) and the shared mind map at
[3.8.13](docs/project/prd.md#L177)) is drawn as its own component rather than as a route on the API.
...
([5.2.2](docs/project/prd.md#L706)): a browser PWA, an iOS App Store build, a Play build.

- **Owns:** playback UI and speed/position state locally, the download queue and progress
  ([3.18.7](docs/project/prd.md#L406)–[3.18.9](docs/project/prd.md#L408)), the offline write outbox
  ([3.18.14](docs/project/prd.md#L419)–[3.18.15](docs/project/prd.md#L420)), optimistic UI.
...
- The **Capacitor shell** contributes exactly three things the browser cannot reliably give us:
  native background audio with lock-screen transport ([3.2.6](docs/project/prd.md#L67)), APNs/FCM push in a
  store build ([5.2.5](docs/project/prd.md#L709)), and a filesystem for downloads that the OS will not evict
  under storage pressure ([3.18](docs/project/prd.md#L391)). It contains no product logic.
...
- **Owns:** authentication and sessions, role enforcement, all member state (progress, notes,
  ...
  publish/review gate ([4.17.3](docs/project/prd.md#L683)), signed-URL minting for media, search query
...
- Exposed as a versioned JSON HTTP API — the API-first requirement in [§6](docs/project/prd.md#L724) is what
...
signed audio enclosures for [5.3.1](docs/project/prd.md#L718), and the single shared-mind-map route at
[3.8.13](docs/project/prd.md#L177). It is separate from the API so that "everything requires auth"
...
- Each step is separately addressable so an admin can re-run one step without the pipeline
  ([3.21.2.4](docs/project/prd.md#L486)), and a failure halts that recording's chain and raises a flag
  rather than continuing on bad input ([3.21.2.3](docs/project/prd.md#L485), [3.5.8](docs/project/prd.md#L119)).

### Media store

Object storage holding original uploads ([3.4.9](docs/project/prd.md#L102)), processed renditions, generated
video, and artwork. Never publicly addressable; every read is a short-lived signed URL issued by the
API after an authorisation check ([§6](docs/project/prd.md#L724) Security).
...
Conceptual only — [§4](docs/project/prd.md#L497) already defines the fields; this is how the entities relate.

**The spine.** `Series 1—* Recording 1—1 Transcript 1—* Segment`. A recording belongs to at most one
series ([3.3.2](docs/project/prd.md#L79)) and can exist without one ([3.3.9](docs/project/prd.md#L86)). Every
`Segment` carries `start_ms`, `end_ms`, text and an embedding vector. `Recording` carries two media
pointers, original and processed ([4.2](docs/project/prd.md#L513)), plus a `PipelineRun` with per-step status.

**Derived artefacts** all hang off the recording and all carry a `status` of draft/published plus
provenance — which model produced it and whether an admin changed it ([4.17.5](docs/project/prd.md#L685)):
`Summary` (one per recording), `ScriptureReference` (structured book/chapter/verse, never free text —
[3.7.3](docs/project/prd.md#L148)), `Tag` applied through a shared taxonomy across recordings and videos
([4.7](docs/project/prd.md#L568)), `MindMap`, and `Video` with a parent recording.
...
([3.9.3](docs/project/prd.md#L187)). It is a cache of the vector index, not a source of truth, and can be
dropped and rebuilt.
...
private content to anyone ([3.10.9](docs/project/prd.md#L208), [3.13.8](docs/project/prd.md#L294),
[3.14.8](docs/project/prd.md#L313), [3.15.9](docs/project/prd.md#L332)).
...
rare), and server-side deletions win — which is how [3.18.12](docs/project/prd.md#L414) removes an unpublished
...
[3.17.10](docs/project/prd.md#L381)–[3.17.12](docs/project/prd.md#L383).
...
([4.5](docs/project/prd.md#L549) *Generated by*). That record is what makes regeneration
([3.6.9](docs/project/prd.md#L135)) and provider migration tractable.

**Outbound publishing.** Each external platform is an adapter behind a common `publish(item, target)`
contract with its own queue and its own status per item ([3.20.7](docs/project/prd.md#L454)). They differ
...
([3.20.8](docs/project/prd.md#L455)), not the mechanics. All are publish-only; nothing is read back
([5.3.5](docs/project/prd.md#L722)).

**Cross-reference recomputation.** [3.9.6](docs/project/prd.md#L190) says references are recomputed when a
...
transition is logged. This is the mechanism behind [4.17.3](docs/project/prd.md#L683) and
[3.21.2.2](docs/project/prd.md#L484), and it is why [3.19.2](docs/project/prd.md#L430)'s Pending Reviews queue is a
...
particular ([3.10.9](docs/project/prd.md#L208)) issues one query whose visibility predicate is `published OR
...
([3.4.11](docs/project/prd.md#L104), [3.5.8](docs/project/prd.md#L119), [3.19.4](docs/project/prd.md#L432)) — it never
publishes partial results. A *request* failure returns a typed error the client can distinguish; the
distinction between "no results" and "search unavailable" at [3.10.11](docs/project/prd.md#L210) is a
...
([3.17.13](docs/project/prd.md#L387)), a push payload dispatched via APNs/FCM in store builds and Web Push in
browsers. Preferences gate push only; the in-app centre receives everything, including muted
categories ([3.17.14](docs/project/prd.md#L388)).
...
([3.19.4](docs/project/prd.md#L432)). Provider spend is tracked per job, because [§7](docs/project/prd.md#L742)
correctly identifies cost as an unknown that needs measuring, not estimating.

**Audit.** An append-only log for external publishes ([3.20.8](docs/project/prd.md#L455)) and for admin
actions on member content ([3.12.10](docs/project/prd.md#L272), [3.16.11](docs/project/prd.md#L357)), carrying actor,
...
profile ([3.4.6](docs/project/prd.md#L99)), video style presets ([3.11.2.2](docs/project/prd.md#L227)), question banks
([3.14.11](docs/project/prd.md#L316)) — are versioned rows in the database, not deploys. That is what makes
[3.4.7](docs/project/prd.md#L100) work: a recording records which profile version processed it, so changing
the profile cannot retroactively alter anything.

## Scalability & growth posture

**Built to stretch.** Content volume, not member count, is the growth axis: [§6](docs/project/prd.md#L724)
...
the API, which is what makes the back-catalogue burst ([3.21.3.3](docs/project/prd.md#L492)) a
...
signals refresh on poll and on push, not over a socket; if the SOS channel ([3.16](docs/project/prd.md#L334))
...
[3.11.4.6](docs/project/prd.md#L246) — which bins a full-cost generation on rejection — becomes an expensive
...
deciding before [3.11](docs/project/prd.md#L212) is scheduled into an epic, because it also determines whether
...
1. **Flow Tracker gap detection vs. "never scored"** ([3.14.6](docs/project/prd.md#L311) against
   [3.14.9](docs/project/prd.md#L314)). Detecting that a response "indicates a gap in understanding" requires
   ...
   intended reading.
2. **Spotify per-episode publishing** ([3.20.4](docs/project/prd.md#L451), [3.20.7](docs/project/prd.md#L454)). No
...
3. **Social publishing prerequisites** ([3.20.3](docs/project/prd.md#L450)). Instagram, TikTok and LinkedIn
...
4. **Bible text licensing** ([3.7.4](docs/project/prd.md#L149)). Determines whether verses render inline or
   link out. The architecture is indifferent — citations are structured either way — but the member
   experience is not.
5. **Two processed renditions, not one** ([3.4.10](docs/project/prd.md#L103)). One processed master, but
   streaming wants a small mono AAC and podcast feeds broadly expect MP3. Treated here as two
   renditions from one processing run, which is a small addition to [4.2](docs/project/prd.md#L513)'s
   *Processed audio* field.
6. **Download ceiling** ([3.18.6](docs/project/prd.md#L405)). Capacitor's native filesystem removes the
...
7. **Embedding dimension** ([3.9](docs/project/prd.md#L179), [3.10](docs/project/prd.md#L194)). At 1536 dimensions, 250k
   ...
   [§3.9](docs/project/prd.md#L179) has shipped. [epic architecture](docs/epics/epic-core-listening/architecture.md#L323) confirms
