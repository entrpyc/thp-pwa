...
[3.9](docs/project/prd.md#L189) is correctly described in the PRD as infrastructure rather than a feature.
...
"a failure in AI generation never blocks listening" ([§6](docs/project/prd.md#L758)) structural rather than
aspirational — and it is also why the one unauthenticated surface in the product (`Feed`, serving
the podcast RSS at [5.3.1](docs/project/prd.md#L752) and the shared mind map at
[3.8.13](docs/project/prd.md#L187)) is drawn as its own component rather than as a route on the API.
...
([5.2.2](docs/project/prd.md#L740)): a browser PWA, an iOS App Store build, a Play build.

- **Owns:** playback UI and speed/position state locally, the download queue and progress
  ([3.18.7](docs/project/prd.md#L416)–[3.18.9](docs/project/prd.md#L418)), the offline write outbox
  ([3.18.14](docs/project/prd.md#L429)–[3.18.15](docs/project/prd.md#L430)), optimistic UI.
...
- The **Capacitor shell** contributes exactly three things the browser cannot reliably give us:
  ...
  store build ([5.2.5](docs/project/prd.md#L743)), and a filesystem for downloads that the OS will not evict
  under storage pressure ([3.18](docs/project/prd.md#L401)). It contains no product logic.
...
- **Owns:** authentication and sessions, role enforcement, all member state (progress, notes,
  ...
  publish/review gate ([4.17.3](docs/project/prd.md#L701)), signed-URL minting for media, search query
...
- Exposed as a versioned JSON HTTP API — the API-first requirement in [§6](docs/project/prd.md#L758) is what
...
signed audio enclosures for [5.3.1](docs/project/prd.md#L752), and the single shared-mind-map route at
[3.8.13](docs/project/prd.md#L187). It is separate from the API so that "everything requires auth"
...
- Each step is separately addressable so an admin can re-run one step without the pipeline
  ([3.21.2.4](docs/project/prd.md#L497)), and a failure halts that recording's chain and raises a flag
  rather than continuing on bad input ([3.21.2.3](docs/project/prd.md#L496), [3.5.8](docs/project/prd.md#L125)).
...
API after an authorisation check ([§6](docs/project/prd.md#L758) Security).
...
Conceptual only — [§4](docs/project/prd.md#L511) already defines the fields; this is how the entities relate.
...
pointers, original and processed ([4.2](docs/project/prd.md#L529)), plus a `PipelineRun` with per-step status.

**Derived artefacts** all hang off the recording and all carry a `status` of draft/published plus
provenance — which model produced it and whether an admin changed it ([4.17.5](docs/project/prd.md#L703)):
...
([4.7](docs/project/prd.md#L586)), `MindMap`, and `Video` with a parent recording.
...
([3.9.3](docs/project/prd.md#L197)). It is a cache of the vector index, not a source of truth, and can be
dropped and rebuilt.
...
private content to anyone ([3.10.9](docs/project/prd.md#L218), [3.13.8](docs/project/prd.md#L304),
[3.14.8](docs/project/prd.md#L323), [3.15.9](docs/project/prd.md#L342)).
...
rare), and server-side deletions win — which is how [3.18.12](docs/project/prd.md#L424) removes an unpublished
...
[3.17.10](docs/project/prd.md#L391)–[3.17.12](docs/project/prd.md#L393).
...
([4.5](docs/project/prd.md#L567) *Generated by*). That record is what makes regeneration
...
contract with its own queue and its own status per item ([3.20.7](docs/project/prd.md#L465)). They differ
...
([3.20.8](docs/project/prd.md#L466)), not the mechanics. All are publish-only; nothing is read back
([5.3.5](docs/project/prd.md#L756)).

**Cross-reference recomputation.** [3.9.6](docs/project/prd.md#L200) says references are recomputed when a
...
transition is logged. This is the mechanism behind [4.17.3](docs/project/prd.md#L701) and
[3.21.2.2](docs/project/prd.md#L495), and it is why [3.19.2](docs/project/prd.md#L440)'s Pending Reviews queue is a
...
particular ([3.10.9](docs/project/prd.md#L218)) issues one query whose visibility predicate is `published OR
...
([3.4.11](docs/project/prd.md#L110), [3.5.8](docs/project/prd.md#L125), [3.19.4](docs/project/prd.md#L442)) — it never
publishes partial results. A *request* failure returns a typed error the client can distinguish; the
distinction between "no results" and "search unavailable" at [3.10.11](docs/project/prd.md#L220) is a
...
([3.17.13](docs/project/prd.md#L397)), a push payload dispatched via APNs/FCM in store builds and Web Push in
browsers. Preferences gate push only; the in-app centre receives everything, including muted
categories ([3.17.14](docs/project/prd.md#L398)).
...
([3.19.4](docs/project/prd.md#L442)). Provider spend is tracked per job, because [§7](docs/project/prd.md#L779)
correctly identifies cost as an unknown that needs measuring, not estimating.

**Audit.** An append-only log for external publishes ([3.20.8](docs/project/prd.md#L466)) and for admin
actions on member content ([3.12.10](docs/project/prd.md#L282), [3.16.11](docs/project/prd.md#L367)), carrying actor,
...
profile ([3.4.6](docs/project/prd.md#L105)), video style presets ([3.11.2.2](docs/project/prd.md#L237)), question banks
([3.14.11](docs/project/prd.md#L326)) — are versioned rows in the database, not deploys. That is what makes
...
**Built to stretch.** Content volume, not member count, is the growth axis: [§6](docs/project/prd.md#L758)
...
the API, which is what makes the back-catalogue burst ([3.21.3.3](docs/project/prd.md#L506)) a
...
signals refresh on poll and on push, not over a socket; if the SOS channel ([3.16](docs/project/prd.md#L344))
...
[3.11.4.6](docs/project/prd.md#L256) — which bins a full-cost generation on rejection — becomes an expensive
...
deciding before [3.11](docs/project/prd.md#L222) is scheduled into an epic, because it also determines whether
...
1. **Flow Tracker gap detection vs. "never scored"** ([3.14.6](docs/project/prd.md#L321) against
   [3.14.9](docs/project/prd.md#L324)). Detecting that a response "indicates a gap in understanding" requires
   ...
   intended reading.
2. **Spotify per-episode publishing** ([3.20.4](docs/project/prd.md#L462), [3.20.7](docs/project/prd.md#L465)). No
...
3. **Social publishing prerequisites** ([3.20.3](docs/project/prd.md#L461)). Instagram, TikTok and LinkedIn
...
5. **Two processed renditions, not one** ([3.4.10](docs/project/prd.md#L109)). One processed master, but
   ...
   renditions from one processing run, which is a small addition to [4.2](docs/project/prd.md#L529)'s
   *Processed audio* field.
6. **Download ceiling** ([3.18.6](docs/project/prd.md#L415)). Capacitor's native filesystem removes the
...
7. **Embedding dimension** ([3.9](docs/project/prd.md#L189), [3.10](docs/project/prd.md#L204)). At 1536 dimensions, 250k
   ...
   [§3.9](docs/project/prd.md#L189) has shipped. [epic architecture](docs/epics/epic-core-listening/architecture.md#L323) confirms
