...
**Deliberately not decided here.** Which speech-to-text, language, embedding, video and text-to-speech providers are used; how media is stored and delivered; the application, API and job-orchestration stack; how the PWA is packaged for the two app stores; the persistence and search infrastructure; and how offline synchronisation resolves conflicts. Each of these has several viable answers, and they belong to the architecture phase.
